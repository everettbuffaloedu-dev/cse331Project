       ...
       //
        ...
        if (cDelay > unsubscribe) {
            ...
            badNodes.remove(client.id);
                sol.paths.set(client,shortestPaths.get(client.id));
                improve(sol, client);
                 bandwidthTracker++;
            }
                improved = true;
        ...
        for(client : this.clients){
            currentPathLengths.put(client.id,sol.paths.get(client.id).size());
        }
        ...
        if(isAtLegalRisk(clientDelays, currentPathLengths) && !complainingClients.isEmpty()){
            ...
            boolean satisfyPossible = complainingsPaths.get(interestId) <= clientOfInterest.beta * shortestPathLengths.get(interestId);//checks if the path currently being used can even satisfy complaining client
         
            if(!satisfyPossible){
                //CODE RED 
                //clean complainingPaths of the path of this client
                sol.paths.set(clientOfInterest,shortestPaths.get(clientOfInterest));
            ...
            if(!searchSuccess){
            sol.paths.set(client,complainingPaths.get(client.id));
            improve(sol, clientOfInterest);
            badNodes.remove(clientOfInterest.id);
             bandwidthTracker++;
            }
